...
async def chat_with_user_websocket(websocket, path):
    ...
    transcriber = ServerTranscriber(DEEPGRAM_API_KEY)
    # try:
    ...
    try:
        async for message in websocket:
            ...
            if "audio" in data:
                # Handle audio data
                audio_chunk = data["audio"]
                transcript = await transcriber.transcribe(bytes.fromhex(audio_chunk))
                if transcript:
                    if transcript.lower() == "exit":
                        print("Exiting as requested by the client")
                        break
                    ...
                    # Handle the case where transcript is None
                    print("No transcript received or an error occurred")        
