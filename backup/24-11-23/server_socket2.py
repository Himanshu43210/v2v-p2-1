...
async def chat_with_user_websocket(websocket, path):
    ...
    # processed_content = ""
    # sentence_end_pattern = re.compile(r"(?<=[.?!])\s")
    # transcriber = Transcriber()  # Initialize the Transcriber
        ...
        async for message in websocket:
            ...
            if "audio" in data:
                ...
                if data.get("end_of_audio", False):
                    # If this is the last chunk of audio, wait for the final transcription
                    transcript = await transcriber.get_final_transcription()
                    if transcript is None:
                        print("Transcription failed or was interrupted.")
                        break
